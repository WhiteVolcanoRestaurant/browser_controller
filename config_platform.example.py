...
# 课程完成/结束页 URL 特征（视频播完/课程学完后从详情页跳到的页，用于可靠判断"课程真的结束"）。
# 不知道时可为空字符串 ""，结束判定将退化为「详情页 → 非详情页」的 URL 变化。
COURSE_FINISH_URL_MARK = "/wk/comment"

# 课程结束页固定正文（该平台结束页必现的 OCR 文本，用于严格匹配判定结束）。
# 不知道时可为空字符串 ""，结束判定将退化为 END_TEXTS 的宽泛匹配。
COURSE_FINISHED_TEXT = "课程的学习已完成"
