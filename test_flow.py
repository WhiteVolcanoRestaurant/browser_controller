...
- 测试页按难度递增：文字按钮、可见图片翻页、完成知识卡片后才显示图片翻页；
  高级样本包含多选题 + 单选题答错重答，有 Ollama 时 VLM 自动作答，无 VLM 时停住等人工作答，
  脚本检测到翻页后自动继续）。另有内容页顶部"返回"干扰项，验证脚本只点底部"下一步"。
- 走到"完成页"（课程的学习已完成）即判定流程跑通，自动退出；Ctrl+C 随时停止，服务器会自动关闭。
"""
...
class QuietHandler(SimpleHTTPRequestHandler):
    ...
    def _reply_progress(self):
        # 测试页每次正常翻页都会请求这个地址，模拟真实平台 next 请求及后台回复。
        if self.path.startswith("/progress/next"):
...
def main():
    ...
    config.PROGRESS_API_MARKS = [{
        "name": "测试翻页",
        "url_marks": ["/progress/next"],
    }]
