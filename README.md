...
写在前面：
一、脚本有调用windows提示音和弹窗，第一次用声音可以关小一点，可以去config里关掉。详见[人工介入提醒](#人工介入提醒可选)。
二、长期挂机（体感大概是一个小时没操作）后整个网页刷新后再学习。长期不操作可能需要重新登录，但是刷新才看得到，如果后端发现你离线了还在发送操作可能会封号一小时。
三、有能力还是配置一个VLM，能大大减少人工介入次数。在线的api如果有人需要再提issue更新吧。
四、本脚本不针对任何平台，使用时有一定门槛，请自行配置[config_platform.py](./config_platform.example.py)（见[适配新平台](#七适配新平台--扩展)），也可通过互动配置脚本[setup_local.py](./setup_local.py)。完整启动流程见[快速开始](#五快速开始开发环境)。
