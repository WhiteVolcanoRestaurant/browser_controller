...
核心分工：**OCR/VLM 理解画面**，**DOM 只观察可见元素和边界**，**Playwright 输入层执行一次点击**。页面自己的处理程序负责生成进度请求，脚本不构造或重放请求正文。
...
| [flow\_state.py](flow_state.py)                           | 显式工作流状态：观察、决策、操作、验证、VLM、人工与完成                              |
...
navigate
  ...
  → click（每轮只操作一个候选；候选失败后重新观察）
      ├─ next 请求已发出且收到响应 → 生效
      └─ 无效 → 可见 btn-next → VLM 语义推理 → 人工
```
