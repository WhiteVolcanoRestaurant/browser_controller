...
### 19、课程列表页自动选择未完成必修课（实验功能）
- **页面证据**：完成课程右上角有绿色角标；导出的 DOM 中对应课程行为
  `li.img-texts-item.passed`。没有绿色角标的课程行不含 `passed`。
- **需求边界**：不区分“从未开始”和“已经开始但未完成”，统一视为未完成；只学习必修课，
  不切换选修课，不进入在线考试。
- **实现**（[browser_controller.py](browser_controller.py) / [main.py](main.py) / [config.py](config.py)）：
  1. 在课程列表路由中读取“必修课”页签进度；必修已完成时直接结束，即使选修仍为 0/60。
  2. 当前不在必修页签时先切回必修；随后查找第一个完成数小于总数的折叠分类。
  3. 分类未展开时先展开；展开后选择第一个不含 `passed` 的课程行。
     分类和课程候选均严格保留页面 DOM 顺序，从上到下选择。
  4. DOM 只返回状态与中心坐标，点击仍由 Playwright 输入层单次执行并验证页面变化。
  5. 课程结束页点击页面自身的“返回课程列表”，回到列表后继续选择下一门必修课。
  6. 分类计数与课程角标矛盾、目标不可见或被遮挡时转人工，不让 VLM 猜测选课目标。
- **开关**：`ENABLE_AUTO_COURSE_SELECTION=True` 默认启用；命令行
  `--manual-course-selection` 可恢复原有人工选课。
- **回归测试**：`test_interaction.py` 覆盖无角标选择、分类展开、从选修切回必修、
  页面从上到下的选择顺序、必修全部完成停止、计数矛盾拒绝点击；同时保留原浏览器输入层与翻页测试。
- **运行记录**：成功进入课程后新增 `course_selected` JSONL 事件，并在终端输出课程名和分类；
  日志同时保存分类/课程顺序、列表页 URL、课程页 URL 与点击验证结果。
